...
@SerialVersionUID(121314583133228662L)
...
class GossipLeaderElection() extends ComponentDefinition {
  ...
  private def checkLeader() {
    ...
    if (topBallot < highestBallot) {
      ...
      if (!leader.contains(top)) {
        ...
        leader = Some(top)
        log.info(s"New leader is $leader");
        trigger(
  ...
  net uponEvent {
    case NetMessage(h, HeartbeatReq(r, hb)) =>
      if (hb > highestBallot) {
        highestBallot = hb
      }
      trigger(
        NetMessage(self, h.getSource(), HeartbeatResp(r, ballot)) -> net
      )
    case NetMessage(h, HeartbeatResp(r, b)) =>
      if (r == round) {
        ballots += (h.getSource() -> b)
      } else {
        period += delta
      }
  }
